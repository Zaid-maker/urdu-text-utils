...
// Normalization
results.push(
  benchmark("normalizeUrdu (short)", () => normalizeUrdu(shortText))
);
results.push(
  benchmark("normalizeUrdu (medium)", () => normalizeUrdu(mediumText))
);
results.push(
  benchmark("normalizeUrdu (long)", () => normalizeUrdu(longText))
);

// Diacritics
results.push(
  benchmark("removeDiacritics (short)", () => removeDiacritics(shortText))
);
results.push(
  benchmark("removeDiacritics (diacritics)", () =>
    removeDiacritics(diacriticText)
  )
);
...
results.push(
  benchmark("romanToUrdu (medium)", () =>
    romanToUrdu("pakistan ek khoobsurat mulk hai")
  )
);

// Search
results.push(
  benchmark("searchUrdu (1000 items, exact)", () =>
    searchUrdu("کراچی", searchCorpus, {
      getText: (r) => r.title,
      limit: 10,
    })
  )
...
results.push(
  benchmark("searchUrdu (1000 items, fuzzy)", () =>
    searchUrdu("کراچی", searchCorpus, {
      getText: (r) => r.title,
      fuzzy: true,
      limit: 10,
...
// Sorting
results.push(
  benchmark("sortUrdu (20 words)", () => sortUrdu([...urduWords]))
);

// Stats
results.push(benchmark("countWords (medium)", () => countWords(mediumText)));
results.push(
  benchmark("countSentences (medium)", () => countSentences(mediumText))
);
...
results.push(
  benchmark("splitSentences (medium)", () => splitSentences(mediumText))
);
...
// Numbers
results.push(
  benchmark("convertNumbers (short)", () => convertNumbers("1234567890"))
);

// Stemming
results.push(
  benchmark("stemUrdu (single word)", () => stemUrdu("کتابیں"))
);
results.push(
  benchmark("stemUrduText (medium)", () => stemUrduText(mediumText))
);

// Date
results.push(
  benchmark("formatUrduDate (now)", () => formatUrduDate(new Date()))
);
results.push(
  benchmark("timeAgoUrdu (2 hours ago)", () =>
    timeAgoUrdu(Date.now() - 2 * 3600 * 1000)
  )
);

// Print results
console.log(
  "Function".padEnd(40) + "Ops/sec".padStart(10) + "  " + "per op".padStart(10)
);
...
for (const r of results) {
  console.log(
    r.name.padEnd(40) +
      r.opsPerSec.toString().padStart(10) +
      "  " +
      (r.perOp + " ms").padStart(10)
  );
}

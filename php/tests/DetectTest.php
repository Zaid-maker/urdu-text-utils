...
final class DetectTest extends TestCase
{
    ...
    {
        $fixtures = json_decode((string) file_get_contents(__DIR__ . '/fixtures/detect.json'), true, 512, JSON_THROW_ON_ERROR);
    ...
    public function testMatchesTypeScriptFixture(array $case): void
    {
        ...
        $actual = match ($case['fn']) {
            'isUrdu' => Detect::isUrdu($case['args'][0], $case['options']),
            'urduRatio' => Detect::urduRatio($case['args'][0]),
            'hasUrduSpecificLetters' => Detect::hasUrduSpecificLetters($case['args'][0]),
        };
